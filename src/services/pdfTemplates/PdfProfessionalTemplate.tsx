...
export const PdfProfessionalTemplate: React.FC<PdfTemplateProps> = ({ resume }) => {
  ...
  const styles = StyleSheet.create({
    ...
    headerBanner: {
      backgroundColor: tokens.accentColor || '#0f172a',
      padding: 16,
      borderRadius: 4,
      marginBottom: 16,
    },
    name: {
      fontSize: tokens.nameFontSizePt * 0.95,
      fontWeight: 'bold',
      color: '#ffffff',
      ...
      {settings.sectionOrder.map((sectionKey) => {
        ...
        if (sectionKey === 'summary' && summary?.trim()) {
          return (
            <View key={sectionKey} style={styles.section} wrap={false}>
              ...
              <Text style={{ paddingHorizontal: 2 }}>{summary}</Text>
            </View>
          );
        }
        ...
        if (sectionKey === 'experience' && experience.length > 0) {
          return (
            <View key={sectionKey} style={styles.section}>
              <Text style={styles.sectionHeading}>{getSectionTitle(sectionKey)}</Text>
              {experience.map((exp) => (
                <View key={exp.id} style={{ marginBottom: 6, paddingHorizontal: 2 }} wrap={false}>
        ...
        if (sectionKey === 'education' && education.length > 0) {
          return (
            <View key={sectionKey} style={styles.section}>
              <Text style={styles.sectionHeading}>{getSectionTitle(sectionKey)}</Text>
              {education.map((edu) => (
                <View key={edu.id} style={{ marginBottom: 4, paddingHorizontal: 2 }} wrap={false}>
                  <View style={styles.itemTitleRow}>
        ...
        if (sectionKey === 'projects' && projects.length > 0) {
          return (
            <View key={sectionKey} style={styles.section}>
              <Text style={styles.sectionHeading}>{getSectionTitle(sectionKey)}</Text>
              {projects.map((proj) => (
                <View key={proj.id} style={{ marginBottom: 4, paddingHorizontal: 2 }} wrap={false}>
                  <View style={styles.itemTitleRow}>
        ...
        if (sectionKey === 'skills' && skills.length > 0) {
          ...
          return (
            <View key={sectionKey} style={styles.section} wrap={false}>
              <Text style={styles.sectionHeading}>{getSectionTitle(sectionKey)}</Text>
              <View style={{ flexDirection: 'row', width: '100%', marginTop: 2, paddingHorizontal: 2 }}>
